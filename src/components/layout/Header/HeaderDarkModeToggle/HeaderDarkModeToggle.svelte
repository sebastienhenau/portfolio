<script lang="ts">
	...
	// -------------------------------------------------------------------------
	onMount(() => {
		const hasDocumentAlreadyDarkMode =
		...
		return () => {
			systemThemeMatcher.removeEventListener(
				"change",
				onSystemThemeChange
			);
		};
...
<svelte:head>
	<script>
		...
		if (
			isLocalThemeDark ||
			(!hasLocalTheme && systemThemeMatcher.matches)
		) {
			document.documentElement.classList.add("dark");
		} else {
			document.documentElement.classList.remove("dark");
...
>
	<span
		class={clsx(
			"absolute z-0 top-0 left-0 w-1/2 h-full bg-border-alt transition-opacity",
			{
	...
	<span
		class={clsx(
			"z-10 flex items-center justify-center flex-1 transition-colors",
			{
